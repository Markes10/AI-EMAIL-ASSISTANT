...
import api, { auth, gmail } from '../utils/api';
import { 
  LockClosedIcon, 
  EnvelopeIcon, 
  EyeIcon, 
...
const AuthForm = () => {
  ...
  useEffect(() => {
    ...
    // Check Gmail connect status if logged in
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
                {gmailStatus?.connected ? (
                  <div className="col-span-3 p-2 border rounded bg-green-50 text-sm text-green-800">
                    Connected to Gmail as <strong>{gmailStatus.email}</strong>
                    <button onClick={() => { gmail.disconnect().then(() => setGmailStatus({connected:false})).catch(()=>{}); }} className="ml-3 text-xs text-red-600">Disconnect</button>
                  </div>
                ) : null}
              <button
                type="button"

...
import api, { emails, resumes, tone as toneApi } from '../utils/api';
import { 
  PaperAirplaneIcon, 
  SparklesIcon,
...
const EmailForm = () => {
  ...
  const handleResumeUpload = async () => {
    ...
    try {
      const resp = await api.post('/api/resume/upload', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      setResumeId(resp.data.resume_id);
  ...
  const handleGenerate = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await emails.generate({
  ...
  const handleAdjustTone = async () => {
    if (!generatedEmail) {
      toast.error('No email to adjust');
      return;
    }

    setAdjustingTone(true);
    try {
      const resp = await toneApi.adjust({ text: generatedEmail, target_tone: formData.tone });
      const adjusted = resp.data.adjusted_text || resp.data.body || generatedEmail;
      setGeneratedEmail(adjusted);
      setEmailHistory(prev => [...prev, adjusted]);
      setCurrentVersion(prev => prev + 1);
      toast.success('Tone adjusted');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Failed to adjust tone');
    } finally {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      formDataToSend.append('body', generatedEmail);
        formDataToSend.append('send_via_gmail', sendViaGmail);
      ...
      await api.post('/api/email/send', formDataToSend, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
              ...
              {showAdvanced && (
                <>
                  ...
                  <div className="flex items-center space-x-2">
                    <input id="sendViaGmail" type="checkbox" checked={sendViaGmail} onChange={(e)=>setSendViaGmail(e.target.checked)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                    <label htmlFor="sendViaGmail" className="text-sm text-gray-700">Send via connected Gmail account</label>
                  </div>
                </>
              )}
              ...
              <div>
                ...
                <div className="mt-1 flex items-center">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${resumeUploading ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    {resumeUploading ? 'Uploading...' : 'Upload Resume'}
                  </button>
                  {resumeId && (
                    <span className="ml-3 text-sm text-green-600">Uploaded (id: {resumeId})</span>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    title="Adjust Tone"
                  >
                    {adjustingTone ? 'Adjusting...' : 'Adjust Tone'}
                  </button>
                  <button
                    type="button"

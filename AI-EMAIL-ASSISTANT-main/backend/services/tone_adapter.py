...
def adjust_tone(text: str, target_tone: str) -> str:
    ...
    """
    # Normalize requested tone
    tone = normalize_tone(target_tone)

    # Build instruction for the LLM
    prompt = (
        f"Rewrite the following email in a {tone.lower()} tone while preserving the original meaning, "
        "keeping greetings and sign-offs intact if present, and not adding any facts. "
        "Keep the length similar and maintain clarity.\n\n"
        f"Original message:\n{text}"
    )

    # Prefer OpenAI ChatCompletion when available
    try:
        ...
        logging.exception("OpenAI tone adjust failed")

    # Fallback: simple heuristic adjustments using tone characteristics
    try:
        chars = get_tone_characteristics(tone)
        text_out = text
        # Prefer replacing discouraged words
        for avoid in chars.get('avoid', [])[:10]:
            text_out = text_out.replace(avoid, '')

        # Add prompt prefix suggestions (non-destructive)
        prefix = get_tone_prompt_prefix(tone)
